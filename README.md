...
python scripts/report.py       # writes docs/findings.md and charts
...
See [`docs/findings.md`](docs/findings.md), regenerated by `scripts/report.py`.

- **47% of U.S. data center moratoria sit in PJM footprint states.** Moratorium data is
  organized by state; grid planning is organized by RTO. Nobody has mapped one onto the other.
- **The source dataset is 98% empty on end dates, but they are computable.** Only 9 of 505
  rows carry a parsed end date; 396 carry a duration. Enacted date plus duration gives a
  usable expiry for 285 of 416 in-force instruments.
- **A forward calendar of when pauses lift.** June 2027 is the peak, with 54 instruments
  scheduled to expire in a single month.
- **35 instruments are listed as in force although their term has already run.** That is the
  Phase 2 verification queue, enumerated rather than estimated.
